...
#import "BTFPeripheralManager.h"
#import "BTFCentral.h"

@interface BTFPeripheralManager()<CBPeripheralManagerDelegate>


@property (nonatomic, strong) NSMutableDictionary * serviceAddingBlocks;
@property (nonatomic, copy) BTFObjectChagedBlock advertisingStartedBlock;

@property (nonatomic, strong) NSDictionary * initialzedOptions;
@property (nonatomic, strong) dispatch_queue_t queue;

@end
@implementation BTFPeripheralManager

#pragma mark - INIT Methods

+(BTFPeripheralManager *)sharedBTFPeripheralManager
{
   static  BTFPeripheralManager *_blePeripheralManagerFactory = nil;
...
//        dispatch_queue_t myQueue = dispatch_queue_create(queueName, DISPATCH_QUEUE_PRIORITY_DEFAULT);
        ...
        _blePeripheralManagerFactory = [[BTFPeripheralManager alloc]initWithQueue:nil options:nil];
    });
...
{
    [[BTFCentral sharedBTFCentral] stopScan];
...
- (void)peripheralManager:(CBPeripheralManager *)peripheral didAddService:(CBService *)service error:(NSError *)error
{
     if (peripheral == self.peripheralManager)
     {
         BTFSpecifiedServiceUpdatedBlock onfinish = self.serviceAddingBlocks[service.UUID];
         if (!error) {
...
- (void)peripheralManagerDidstartAdvertisingBTF:(CBPeripheralManager *)peripheral error:(NSError *)error
{
    if (peripheral == self.peripheralManager)
    {
