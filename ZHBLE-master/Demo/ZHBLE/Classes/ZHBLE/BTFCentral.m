...
@interface BTFCentral()<CBCentralManagerDelegate,CBPeripheralDelegate>
@property (nonatomic, copy) BTFPeripheralDisConnectionBlock canCelConnectionBlock;
@end


#pragma mark -Life cycle
@implementation BTFCentral

+(BTFCentral *)sharedBTFCentral
{
    static BTFCentral *_bleCentralFactory = nil;
    ...
    dispatch_once(&oncePredicate,^{
        ...
        _bleCentralFactory = [[BTFCentral alloc]initWithQueue:nil options:opts];
    });
...
-(void)initializeWithQueue:(dispatch_queue_t) queue options:(NSDictionary *) options
{
    ...
    [BTFStoredPeripherals initializeStorage];//初始化存储
...
-(void)scanPeripheralWithServicesBTF:(NSArray *)serviceUUIDs options:(NSDictionary *)options onUpdated:(BTFPeripheralUpdatedBlock)onUpdateBlock
{
    ...
    if (serviceUUIDs) {
        ...
        [array enumerateObjectsUsingBlock:^(CBPeripheral *peripheral, NSUInteger index, BOOL *stop){
            BTFPeripheral *zhPeripheral = peripheral.delegate;
            if (!zhPeripheral) {
                zhPeripheral = [[BTFPeripheral alloc] initWithPeripheral:peripheral];
            }
...
-(NSArray *)retrieveConnectedPeripheralsWithServicesBTF:(NSArray *)serviceUUIDs
{
    ...
    for (CBPeripheral * peri in ZHPeripherals) {
        BTFPeripheral *zhperi = peri.delegate;
        if (!zhperi) {
            zhperi = [[BTFPeripheral alloc]initWithPeripheral:peri];
        }
...
-(NSArray *)retrievePeriphearlsWithIdentifiersBTF:(NSArray *)identifiers
{
...
{
    BTFPeripheral *thePeripheral = peripheral.delegate;
    if (thePeripheral && [self.connectingPeripherals containsObject:thePeripheral]) {
        BTFPeripheralConnectionBlock finish = self.connectionFinishBlocks[peripheral.identifier];
        ...
        BTFManager *manager = [BTFManager sharedBTFManager];
        manager.connectPeripheral = peripheral;
        
        //Stored to local
        [BTFStoredPeripherals saveUUIDBTF:peripheral.identifier];
...
{
    BTFPeripheral *thePeripheral = peripheral.delegate;
    BTFPeripheralConnectionBlock finish = self.connectionFinishBlocks[thePeripheral.identifier];
    if (finish) {
