//
//  BTFPeripheralTableViewController.m
//  BTF
//
...
#import "BTFPeripheralTableViewController.h"
#import "Constant.h"
#import "UIAlertView+showAlertView.h"


@interface BTFPeripheralTableViewController (){
    NSArray *titles;
}

@end

@implementation BTFPeripheralTableViewController
...
- (void)viewDidLoad {
    ...
    self.peripheralManager = [BTFPeripheralManager sharedBTFPeripheralManager];
    ...
    [button addTarget:self action:@selector(BTFclickRightItem:) forControlEvents:UIControlEventTouchUpInside];
    [button sizeToFit];
...
-(void)BTFclickRightItem:(id)sender
{
    UIButton *button = sender;
    if (!button.selected) {
        [self startAdvertisingBTF];
    }else{
        [self stopAdvertisingBTF];
    }
...
-(void)startAdvertisingBTF
{
    if (!self.peripheralManager.isAdvertizing) {
        ...
        [self.peripheralManager startAdvertisingBTF:temServiceDic onStarted:^(NSError *error){
            NSString *message = nil;
            if (!error) {
...
{
    if (self.peripheralManager.isAdvertizing) {
         [self.peripheralManager stopAdvertisingBTF];
    }
}
